...
/**
 * Fetch the most recent confirmation link for the given email from MailHog.
 */
...
test("register, confirm via MailHog, and log in", async ({ page, request }) => {
  ...
  // Register new user.
  // Wait for the page to fully settle (Blazor circuit connected) before typing —
  // otherwise a re-render can reset the inputs to server state mid-typing.
  ...
  const confirmLink = await getConfirmationLink(request, testEmail);
